// Code generated by gorm.io/gen. DO NOT EDIT.
// Code generated by gorm.io/gen. DO NOT EDIT.
// Code generated by gorm.io/gen. DO NOT EDIT.

package gen_query

import (
	"context"
	"jim_service/internal/gen_model"

	"gorm.io/gorm"
	"gorm.io/gorm/clause"
	"gorm.io/gorm/schema"

	"gorm.io/gen"
	"gorm.io/gen/field"
)

func newGroupUser(db *gorm.DB) groupUser {
	_groupUser := groupUser{}

	_groupUser.groupUserDo.UseDB(db)
	_groupUser.groupUserDo.UseModel(&gen_model.GroupUser{})

	tableName := _groupUser.groupUserDo.TableName()
	_groupUser.ALL = field.NewField(tableName, "*")
	_groupUser.ID = field.NewUint64(tableName, "id")
	_groupUser.GroupID = field.NewUint64(tableName, "group_id")
	_groupUser.UserID = field.NewUint64(tableName, "user_id")
	_groupUser.UserShowName = field.NewString(tableName, "user_show_name")
	_groupUser.Extra = field.NewString(tableName, "extra")
	_groupUser.CreatedAt = field.NewTime(tableName, "created_at")
	_groupUser.UpdatedAt = field.NewTime(tableName, "updated_at")
	_groupUser.DeletedAt = field.NewField(tableName, "deleted_at")

	_groupUser.fillFieldMap()

	return _groupUser
}

type groupUser struct {
	groupUserDo groupUserDo

	ALL          field.Field
	ID           field.Uint64
	GroupID      field.Uint64
	UserID       field.Uint64
	UserShowName field.String
	Extra        field.String
	CreatedAt    field.Time
	UpdatedAt    field.Time
	DeletedAt    field.Field

	fieldMap map[string]field.Expr
}

func (g groupUser) As(alias string) *groupUser {
	g.groupUserDo.DO = *(g.groupUserDo.As(alias).(*gen.DO))

	g.ALL = field.NewField(alias, "*")
	g.ID = field.NewUint64(alias, "id")
	g.GroupID = field.NewUint64(alias, "group_id")
	g.UserID = field.NewUint64(alias, "user_id")
	g.UserShowName = field.NewString(alias, "user_show_name")
	g.Extra = field.NewString(alias, "extra")
	g.CreatedAt = field.NewTime(alias, "created_at")
	g.UpdatedAt = field.NewTime(alias, "updated_at")
	g.DeletedAt = field.NewField(alias, "deleted_at")

	g.fillFieldMap()

	return &g
}

func (g *groupUser) WithContext(ctx context.Context) *groupUserDo {
	return g.groupUserDo.WithContext(ctx)
}

func (g groupUser) TableName() string { return g.groupUserDo.TableName() }

func (g *groupUser) GetFieldByName(fieldName string) (field.Expr, bool) {
	field, ok := g.fieldMap[fieldName]
	return field, ok
}

func (g *groupUser) fillFieldMap() {
	g.fieldMap = make(map[string]field.Expr, 8)
	g.fieldMap["id"] = g.ID
	g.fieldMap["group_id"] = g.GroupID
	g.fieldMap["user_id"] = g.UserID
	g.fieldMap["user_show_name"] = g.UserShowName
	g.fieldMap["extra"] = g.Extra
	g.fieldMap["created_at"] = g.CreatedAt
	g.fieldMap["updated_at"] = g.UpdatedAt
	g.fieldMap["deleted_at"] = g.DeletedAt
}

func (g groupUser) clone(db *gorm.DB) groupUser {
	g.groupUserDo.ReplaceDB(db)
	return g
}

type groupUserDo struct{ gen.DO }

func (g groupUserDo) Debug() *groupUserDo {
	return g.withDO(g.DO.Debug())
}

func (g groupUserDo) WithContext(ctx context.Context) *groupUserDo {
	return g.withDO(g.DO.WithContext(ctx))
}

func (g groupUserDo) Clauses(conds ...clause.Expression) *groupUserDo {
	return g.withDO(g.DO.Clauses(conds...))
}

func (g groupUserDo) Not(conds ...gen.Condition) *groupUserDo {
	return g.withDO(g.DO.Not(conds...))
}

func (g groupUserDo) Or(conds ...gen.Condition) *groupUserDo {
	return g.withDO(g.DO.Or(conds...))
}

func (g groupUserDo) Select(conds ...field.Expr) *groupUserDo {
	return g.withDO(g.DO.Select(conds...))
}

func (g groupUserDo) Where(conds ...gen.Condition) *groupUserDo {
	return g.withDO(g.DO.Where(conds...))
}

func (g groupUserDo) Order(conds ...field.Expr) *groupUserDo {
	return g.withDO(g.DO.Order(conds...))
}

func (g groupUserDo) Distinct(cols ...field.Expr) *groupUserDo {
	return g.withDO(g.DO.Distinct(cols...))
}

func (g groupUserDo) Omit(cols ...field.Expr) *groupUserDo {
	return g.withDO(g.DO.Omit(cols...))
}

func (g groupUserDo) Join(table schema.Tabler, on ...field.Expr) *groupUserDo {
	return g.withDO(g.DO.Join(table, on...))
}

func (g groupUserDo) LeftJoin(table schema.Tabler, on ...field.Expr) *groupUserDo {
	return g.withDO(g.DO.LeftJoin(table, on...))
}

func (g groupUserDo) RightJoin(table schema.Tabler, on ...field.Expr) *groupUserDo {
	return g.withDO(g.DO.RightJoin(table, on...))
}

func (g groupUserDo) Group(cols ...field.Expr) *groupUserDo {
	return g.withDO(g.DO.Group(cols...))
}

func (g groupUserDo) Having(conds ...gen.Condition) *groupUserDo {
	return g.withDO(g.DO.Having(conds...))
}

func (g groupUserDo) Limit(limit int) *groupUserDo {
	return g.withDO(g.DO.Limit(limit))
}

func (g groupUserDo) Offset(offset int) *groupUserDo {
	return g.withDO(g.DO.Offset(offset))
}

func (g groupUserDo) Scopes(funcs ...func(gen.Dao) gen.Dao) *groupUserDo {
	return g.withDO(g.DO.Scopes(funcs...))
}

func (g groupUserDo) Unscoped() *groupUserDo {
	return g.withDO(g.DO.Unscoped())
}

func (g groupUserDo) Create(values ...*gen_model.GroupUser) error {
	if len(values) == 0 {
		return nil
	}
	return g.DO.Create(values)
}

func (g groupUserDo) CreateInBatches(values []*gen_model.GroupUser, batchSize int) error {
	return g.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (g groupUserDo) Save(values ...*gen_model.GroupUser) error {
	if len(values) == 0 {
		return nil
	}
	return g.DO.Save(values)
}

func (g groupUserDo) First() (*gen_model.GroupUser, error) {
	if result, err := g.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*gen_model.GroupUser), nil
	}
}

func (g groupUserDo) Take() (*gen_model.GroupUser, error) {
	if result, err := g.DO.Take(); err != nil {
		return nil, err
	} else {
		return result.(*gen_model.GroupUser), nil
	}
}

func (g groupUserDo) Last() (*gen_model.GroupUser, error) {
	if result, err := g.DO.Last(); err != nil {
		return nil, err
	} else {
		return result.(*gen_model.GroupUser), nil
	}
}

func (g groupUserDo) Find() ([]*gen_model.GroupUser, error) {
	result, err := g.DO.Find()
	return result.([]*gen_model.GroupUser), err
}

func (g groupUserDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*gen_model.GroupUser, err error) {
	buf := make([]*gen_model.GroupUser, 0, batchSize)
	err = g.DO.FindInBatches(&buf, batchSize, func(tx gen.Dao, batch int) error {
		defer func() { results = append(results, buf...) }()
		return fc(tx, batch)
	})
	return results, err
}

func (g groupUserDo) FindInBatches(result *[]*gen_model.GroupUser, batchSize int, fc func(tx gen.Dao, batch int) error) error {
	return g.DO.FindInBatches(result, batchSize, fc)
}

func (g groupUserDo) Attrs(attrs ...field.AssignExpr) *groupUserDo {
	return g.withDO(g.DO.Attrs(attrs...))
}

func (g groupUserDo) Assign(attrs ...field.AssignExpr) *groupUserDo {
	return g.withDO(g.DO.Assign(attrs...))
}

func (g groupUserDo) Joins(field field.RelationField) *groupUserDo {
	return g.withDO(g.DO.Joins(field))
}

func (g groupUserDo) Preload(field field.RelationField) *groupUserDo {
	return g.withDO(g.DO.Preload(field))
}

func (g groupUserDo) FirstOrInit() (*gen_model.GroupUser, error) {
	if result, err := g.DO.FirstOrInit(); err != nil {
		return nil, err
	} else {
		return result.(*gen_model.GroupUser), nil
	}
}

func (g groupUserDo) FirstOrCreate() (*gen_model.GroupUser, error) {
	if result, err := g.DO.FirstOrCreate(); err != nil {
		return nil, err
	} else {
		return result.(*gen_model.GroupUser), nil
	}
}

func (g groupUserDo) FindByPage(offset int, limit int) (result []*gen_model.GroupUser, count int64, err error) {
	count, err = g.Count()
	if err != nil {
		return
	}

	result, err = g.Offset(offset).Limit(limit).Find()
	return
}

func (g groupUserDo) ScanByPage(result interface{}, offset int, limit int) (count int64, err error) {
	count, err = g.Count()
	if err != nil {
		return
	}

	err = g.Offset(offset).Limit(limit).Scan(result)
	return
}

func (g *groupUserDo) withDO(do gen.Dao) *groupUserDo {
	g.DO = *do.(*gen.DO)
	return g
}
