...
func newGroup(db *gorm.DB) group {
	...
	_group.groupDo.UseModel(&gen_model.Group{})
...
func (a groupManyToManyGroupUser) Model(m *gen_model.Group) *groupManyToManyGroupUserTx {
...
func (a groupManyToManyGroupUserTx) Find() (result *gen_model.GroupUser, err error) {
	return result, a.tx.Find(&result)
}

func (a groupManyToManyGroupUserTx) Append(values ...*gen_model.GroupUser) (err error) {
...
func (a groupManyToManyGroupUserTx) Replace(values ...*gen_model.GroupUser) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a groupManyToManyGroupUserTx) Delete(values ...*gen_model.GroupUser) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (g groupDo) Create(values ...*gen_model.Group) error {
	if len(values) == 0 {
		return nil
	}
...
func (g groupDo) CreateInBatches(values []*gen_model.Group, batchSize int) error {
	return g.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (g groupDo) Save(values ...*gen_model.Group) error {
	if len(values) == 0 {
		return nil
	}
...
func (g groupDo) First() (*gen_model.Group, error) {
	if result, err := g.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*gen_model.Group), nil
	}
}

func (g groupDo) Take() (*gen_model.Group, error) {
...
func (g groupDo) Last() (*gen_model.Group, error) {
...
func (g groupDo) Find() ([]*gen_model.Group, error) {
	result, err := g.DO.Find()
	return result.([]*gen_model.Group), err
}

func (g groupDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*gen_model.Group, err error) {
	buf := make([]*gen_model.Group, 0, batchSize)
...
func (g groupDo) FindInBatches(result *[]*gen_model.Group, batchSize int, fc func(tx gen.Dao, batch int) error) error {
...
func (g groupDo) FirstOrInit() (*gen_model.Group, error) {
...
func (g groupDo) FirstOrCreate() (*gen_model.Group, error) {
...
func (g groupDo) FindByPage(offset int, limit int) (result []*gen_model.Group, count int64, err error) {
	count, err = g.Count()
	if err != nil {
		return
	}
