...
// WithUserShowname user_showname获取 用户在群组的昵称
func (obj *_GroupUserMgr) WithUserShowname(userShowname string) Option {
	return optionFunc(func(o *options) { o.query["user_showname"] = userShowname })
}
...
// GetFromUserShowname 通过user_showname获取内容 用户在群组的昵称
func (obj *_GroupUserMgr) GetFromUserShowname(userShowname string) (results []*GroupUser, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(GroupUser{}).Where("`user_showname` = ?", userShowname).Find(&results).Error
...
func (obj *_GroupUserMgr) GetBatchFromUserShowname(userShownames []string) (results []*GroupUser, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(GroupUser{}).Where("`user_showname` IN (?)", userShownames).Find(&results).Error
